-- 075_fix_billing_cycles_boundaries.sql
-- 統一 billing cycles 函數使用 get_contract_end_exclusive
--
-- 背景：
-- - 069 已修正期數計算用 generate_series + period_start < end_date
-- - 但 069 假設 end_date 永遠是排他的，沒有考慮舊資料語義不一致
-- - 073 建立了 get_contract_end_exclusive 動態推導排他邊界
--
-- 本 migration：
-- - 讓 get_contract_billing_cycles 使用 get_contract_end_exclusive
-- - 讓 get_contract_billing_summary 使用 get_contract_end_exclusive
-- - 統一所有邊界判斷邏輯
--
-- Date: 2025-12-29
...
BEGIN
    -- 測試 DZ-058 (contract_id = 555) 如果存在
    SELECT get_contract_billing_summary(555) INTO v_test_summary;

    IF v_test_summary IS NOT NULL AND (v_test_summary->>'is_billable')::BOOLEAN THEN
        RAISE NOTICE '=== DZ-058 (contract_id=555) 驗證 ===';
        RAISE NOTICE 'total_periods: % (應為 4)', v_test_summary->>'total_periods';
        RAISE NOTICE 'paid_periods: %', v_test_summary->>'paid_periods';
        RAISE NOTICE 'overdue_periods: % (應為 0)', v_test_summary->>'overdue_periods';
        RAISE NOTICE 'next_due_date: % (應為 NULL，合約已結束)', v_test_summary->>'next_due_date';
    END IF;

    RAISE NOTICE '';
    RAISE NOTICE '=== Migration 075 完成 ===';
    RAISE NOTICE '✅ get_contract_billing_cycles 已使用 get_contract_end_exclusive';
    RAISE NOTICE '✅ get_contract_billing_summary 已使用 get_contract_end_exclusive';
    RAISE NOTICE '';
    RAISE NOTICE '統一邊界邏輯：';
    RAISE NOTICE '- 同日（如 12/7 ~ 12/7）→ 視為排他邊界';
    RAISE NOTICE '- 不同日（如 12/7 ~ 12/6）→ 視為含當日，排他邊界 +1 天';
    RAISE NOTICE '- 兼容新舊資料，無需修改現有合約';
END $$;
