-- 054_contract_billing_cycles.sql
-- 合約繳費週期視圖：顯示合約期間的所有應繳期數及其狀態
--
-- 解決問題：
-- ContractWorkspace 只顯示「首期收款」，看不到後續週期性繳費進度
--
-- 實作方式：
-- 1. get_contract_billing_cycles: 用 generate_series 產出期數，LEFT JOIN payments
-- 2. 修改 get_contract_timeline: 加入 billing_summary

-- ============================================================================
-- 1. 建立 get_contract_billing_cycles 函數
-- ============================================================================
...
BEGIN
    ...
    IF v_contract.end_date IS NOT NULL THEN
        v_total_periods := CEIL(
            EXTRACT(EPOCH FROM (v_contract.end_date - v_contract.start_date)) /
    ...
    v_current_period_idx := CEIL(
        EXTRACT(EPOCH FROM (CURRENT_DATE - v_contract.start_date)) /
        EXTRACT(EPOCH FROM v_cycle_interval)
    )::INT;
...
COMMENT ON FUNCTION get_contract_billing_cycles IS '取得合約繳費週期列表（含付款/發票狀態）';

GRANT EXECUTE ON FUNCTION get_contract_billing_cycles TO anon, authenticated;

-- ============================================================================
-- 2. 建立 get_contract_billing_summary 函數（供 Timeline 使用）
-- ============================================================================
...
COMMENT ON FUNCTION get_contract_billing_summary IS '取得合約繳費週期摘要（已繳/待繳/逾期/未建立期數）';

GRANT EXECUTE ON FUNCTION get_contract_billing_summary TO anon, authenticated;

-- ============================================================================
-- 3. 修改 get_contract_timeline 加入 billing_summary
-- ============================================================================

-- 先檢查現有函數簽名，避免破壞
DO $$
BEGIN
    RAISE NOTICE '=== Migration 054 完成 ===';
    RAISE NOTICE '✅ get_contract_billing_cycles 函數已建立';
    RAISE NOTICE '✅ get_contract_billing_summary 函數已建立';
    RAISE NOTICE '提示：前端需要呼叫這兩個 RPC 來顯示繳費週期';
END $$;
